...
console.log(e.key)
//const playerLeftPos = Array.from(player.style.left).splice(0, player.style.left.length - 2).join("")
let playerLeftPos ='';
...
switch(e.key){
    case 'd': {player.style.left = `${parseInt(playerLeftPos) + 50}px`;
    console.log('it fired');
    break;
}
case 'a': {player.style.left = `${parseInt(playerLeftPos) - 50}px`;
    console.log('it fired');
    break;
}
}
